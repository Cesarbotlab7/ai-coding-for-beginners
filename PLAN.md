...
- [ ] 更新 `BaseLayout.astro` 的品牌、SEO 和跳转链接。
- [ ] 更新 `ChapterLayout.astro` 的阶段、版本与导航。
- [ ] 重建首页和本地进度。
- [ ] 重构通用 `ChapterCloser.astro`，移除专用包装器。
- [ ] 完成全局组件样式和 reduced-motion。
...
- [ ] 删除旧课程数据。
- [ ] 更新 `README.md`。
- [ ] 在 `docs/decisions.md` 追加取代旧课程方向的 ADR。
- [ ] 确认用户文件未进入暂存区。
